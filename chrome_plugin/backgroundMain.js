...
chrome.runtime.onMessage.addListener(function(msg, msgSender, sendResponse){
  ...
  switch(msg.e){
    ...
    case 'zbGetExtDetails':
      ...
      break;
    // case 'zbgetAndUpdateUserInfoOrShowAuthentication':
    //   console.log('zbgetAndUpdateUserInfoOrShowAuthentication');
    //   getAndUpdateUserInfo(function(result) {
    //     if(!result.error && sendResponse != 'undefined'){
...
function getAndUpdateUserInfo (coptions, callback) {
  console.log('getAndUpdateUserInfo');
  getGoogleAuthToken(options, function(access_token){
    retryConnection.hold('onGotAuthTokenSendToServer'+access_token, 5*1000, function(){
      onGotAuthTokenSendToServer(access_token, callback);  
    });
  });
}
  ...
  var backgroundMessageHandler = function(msg, port) {
    ...
    switch(msg.e){
      case 'UserInit':
        ...
        xhRequest("GET", zbBaseURL + authUserPath, function(result) {
          console.log(result);
      ...
      case 'getAndUpdateUserInfoOrShowAuthentication':
        console.log('getAndUpdateUserInfoOrShowAuthentication');
        getAndUpdateUserInfo({interactive:true}, function(result) {
          if(!result.error){
